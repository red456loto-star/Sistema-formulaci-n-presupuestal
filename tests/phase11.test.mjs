...
async function call(server, method, route, body) {
  const response = await fetch(`${server.url}${route}`, { method, headers, body: body === undefined ? undefined : JSON.stringify(body) });
  return { response, body: await response.json().catch(() => null) };
}

function context(ids, periodId, budgetTypeId) {
  return { company_id: ids.companyId, exercise_id: ids.exerciseId, period_id: periodId, version_id: ids.originalVersionId, budget_type_id: budgetTypeId };
}

function row(values) {
  return {
    center_code: "C-F9-A", center_name: "Centro A", element_code: "EL-F9", element_name: "Elemento F9",
    account_code: null, account_name: values.line_name, account_nature: values.account_nature,
    line_code: values.financial_item, line_name: values.line_name, statement_section: values.statement_section ?? "ESTADO_RESULTADOS",
    financial_item: values.financial_item, cost_behavior: values.cost_behavior ?? "NO_APLICA", cost_traceability: values.cost_traceability ?? "NO_APLICA",
    quantity: null, unit_price: null, amount: values.amount, source_reference: "Fixture sintético", notes: "Dato demostrativo no oficial",
  };
}
...
function importBody(ids, periodId, budgetTypeId, dataKind, rows, extra = {}) {
  return {
    ...context(ids, periodId, budgetTypeId), data_kind: dataKind,
    source_file: `${dataKind.toLowerCase()}-corr2.xlsx`, source_label: "Fuente demostrativa", source_url: "https://example.test/fixture",
    source_period: "2041-01", operator_name: "Responsable de prueba", wacc_rate: 10,
    notes: "Información sintética para pruebas automatizadas", replace_existing: false, rows, ...extra,
  };
}
...
test("Corr 2 mantiene jerarquía, separa tipos de presupuesto y conserva análisis", async (t) => {
  ...
  const inspected = await call(server, "POST", "/api/phase11/master-data/inspect", { file_name: template.body.file_name, content_base64: template.body.content_base64 });
  ...
  const append = await call(server, "POST", "/api/phase11/master-data/import", { ...importBody(ids, period.id, budgetTypeId, "PRESUPUESTADO", [row({ line_name: "Partida manual adicional", account_nature: "GASTO", financial_item: null, statement_section: "PRESUPUESTO", amount: 25, cost_behavior: "FIJO", cost_traceability: "INDIRECTO" })]), source_file: null });
  ...
  const proposal = await call(server, "POST", "/api/phase11/proposals", {
    ...ctx, center_id: suggestion.center_id, element_id: suggestion.element_id, account_id: suggestion.account_id,
    problem: suggestion.problem, evidence_value: suggestion.evidence_value, evidence_unit: suggestion.evidence_unit,
    ...
    expected_impact: suggestion.expected_impact, profitability_impact: suggestion.profitability_impact, responsible_id: suggestion.responsible_id,
    priority: suggestion.priority, due_date: suggestion.due_date, status: "PROPUESTA",
  });
  ...
  const sent = await call(server, "POST", "/api/phase10/email/send", {
    company_id: ids.companyId, exercise_id: ids.exerciseId, version_id: ids.originalVersionId, center_id: ids.centerA, period_number: 1,
