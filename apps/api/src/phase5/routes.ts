...
import {
  ensureEditableLine, getOriginalLine, getOriginalVersion, getPeriods, getProjectionYears,
  nullableNumber, resolveDimensions, roundValue,
} from "./common";

const finiteNumber = z.coerce.number().finite();
const nullableFinite = z.union([finiteNumber, z.null()]).optional();
...
const projectSchema = z.object({ rates: z.array(finiteNumber.min(-100).max(1000)).length(3) });
...
function getDetailedLine(database: DatabaseManager, lineId: number) {
  ...
  const monthly = database.connection.prepare(`SELECT mv.*, p.period_number, p.name AS period_name, p.status AS period_status,
  ...
  const realValues = monthly.map((item) => nullableNumber(item.real_value)).filter((value): value is number => value !== null);
  const annualReal = realValues.length ? realValues.reduce((sum, value) => sum + value, 0) : null;
  return {
    ...line,
    monthly_values: monthly,
    projections,
    annual_budgeted: roundValue(annualBudgeted, Number(line.currency_decimals ?? 2)),
    annual_real: annualReal === null ? null : roundValue(annualReal, Number(line.currency_decimals ?? 2)),
    annual_variance: annualReal === null ? null : roundValue(annualReal - annualBudgeted, Number(line.currency_decimals ?? 2)),
    complete: monthly.length === 12 && projections.length === 3,
  };
}

function listLines(database: DatabaseManager, companyId: number, exerciseId: number, versionId: number, request: Request) {
  ...
  const filters: Array<[string, unknown]> = [
    ["center_id", request.query.center_id], ["group_id", request.query.group_id],
    ["element_id", request.query.element_id], ["account_id", request.query.account_id],
  ];
  for (const [field, raw] of filters) {
    const value = numberQuery(raw);
    if (value) { conditions.push(`${field}=?`); params.push(value); }
  }
  const ids = database.connection.prepare(`SELECT id FROM budget_original_lines WHERE ${conditions.join(" AND ")} ORDER BY center_id, group_id, element_id, account_id`).all(...params) as Array<{ id: number }>;
  return ids.map((item) => getDetailedLine(database, item.id));
}
...
export function registerOriginalBudgetRoutes(app: Express, database: DatabaseManager) {
  app.get("/api/budget-original/lines", (request: Request, response: Response) => {
    ...
    response.json(listLines(database, companyId, exerciseId, versionId, request));
  });
  ...
  app.get("/api/budget-original/summary", (request: Request, response: Response) => {
    ...
    const exerciseId = Number(request.query.exercise_id);
    const versionId = Number(request.query.version_id);
    const lines = listLines(database, companyId, exerciseId, versionId, request) as Array<Record<string, unknown>>;
    ...
    const totalReal = linesWithReal.length ? linesWithReal.reduce((sum, line) => sum + Number(line.annual_real ?? 0), 0) : null;
    response.json({
  ...
  app.get("/api/budget-original/lines/:id", (request: Request, response: Response) => {
    response.json(getDetailedLine(database, Number(request.params.id)));
  });
  ...
  app.post("/api/budget-original/lines", (request: Request, response: Response) => {
    ...
    const projections = getProjectionYears(database, input.company_id, input.exercise_id);
    ...
    const id = database.connection.transaction(() => {
      const result = database.connection.prepare(`INSERT INTO budget_original_lines
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .run(input.company_id, input.exercise_id, input.version_id, input.center_id, dimensions.groupId, dimensions.elementId, input.account_id, input.currency_id, input.unit_id ?? null, input.responsible_id ?? null, input.comment ?? null, input.support ?? null, input.source ?? null, stamp, stamp);
      const lineId = Number(result.lastInsertRowid);
      const insertMonth = database.connection.prepare("INSERT INTO budget_original_monthly_values (line_id,period_id,budgeted_value,real_value,created_at,updated_at) VALUES (?,?,0,NULL,?,?)");
      periods.forEach((period) => insertMonth.run(lineId, period.id, stamp, stamp));
      const insertProjection = database.connection.prepare("INSERT INTO budget_original_projections (line_id,projection_year_id,budgeted_value,real_value,created_at,updated_at) VALUES (?,?,0,NULL,?,?)");
      projections.forEach((projection) => insertProjection.run(lineId, projection.id, stamp, stamp));
      return lineId;
    })();
  ...
  app.patch("/api/budget-original/lines/:id", (request: Request, response: Response) => {
    ...
    const dimensions = resolveDimensions(database, Number(before.company_id), centerId, accountId, currencyId, unitId, responsibleId);
    const periods = new Map(getPeriods(database, Number(before.company_id), Number(before.exercise_id)).map((item) => [Number(item.id), item]));
    const projectionYears = new Set(getProjectionYears(database, Number(before.company_id), Number(before.exercise_id)).map((item) => Number(item.id)));
    const stamp = new Date().toISOString();
    ...
    database.connection.transaction(() => {
      for (const value of input.monthly_values ?? []) {
        ...
        if (changed && String(period.status) === "CERRADO") httpError(`El periodo ${period.name} está cerrado y no puede modificarse.`, 409);
      ...
      for (const value of input.projections ?? []) {
        if (!projectionYears.has(value.projection_year_id)) httpError("Uno de los años proyectados no pertenece al ejercicio activo.");
        const current = database.connection.prepare("SELECT * FROM budget_original_projections WHERE line_id=? AND projection_year_id=?").get(id, value.projection_year_id) as Record<string, unknown> | undefined;
      ...
      database.connection.prepare(`UPDATE budget_original_lines SET center_id=?,group_id=?,element_id=?,account_id=?,currency_id=?,unit_id=?,responsible_id=?,comment=?,support=?,source_text=?,updated_at=? WHERE id=?`)
        .run(centerId, dimensions.groupId, dimensions.elementId, accountId, currencyId, unitId, responsibleId,
    ...
    audit(database, "EDITAR", "budget_original_lines", id, Number(before.company_id), "Línea de presupuesto original actualizada.", before, input);
  ...
  app.post("/api/budget-original/lines/:id/distribute", (request: Request, response: Response) => {
    ...
    const closedTotal = values.filter((item) => item.status === "CERRADO").reduce((sum, item) => sum + Number(item.budgeted_value), 0);
    ...
    database.connection.transaction(() => {
      open.forEach((item, index) => {
        const amount = index === open.length - 1 ? roundValue(remaining - assigned, currency.decimals) : share;
        assigned += amount;
        database.connection.prepare("UPDATE budget_original_monthly_values SET budgeted_value=?,updated_at=? WHERE id=?").run(amount, stamp, item.id);
      });
  ...
  app.post("/api/budget-original/lines/:id/project", (request: Request, response: Response) => {
    ...
    const line = ensureEditableLine(database, id);
    const currency = database.connection.prepare("SELECT decimals FROM currencies WHERE id=?").get(line.currency_id) as { decimals: number };
    const annual = (database.connection.prepare("SELECT COALESCE(SUM(budgeted_value),0) AS total FROM budget_original_monthly_values WHERE line_id=?").get(id) as { total: number }).total;
    const rows = database.connection.prepare(`SELECT bp.id, py.sequence FROM budget_original_projections bp
      JOIN projection_years py ON py.id=bp.projection_year_id WHERE bp.line_id=? ORDER BY py.sequence`).all(id) as Array<{ id: number; sequence: number }>;
    if (rows.length !== 3) httpError("La línea no contiene los tres años de proyección.", 409);
    let value = Number(annual);
    const stamp = new Date().toISOString();
    database.connection.transaction(() => {
      rows.forEach((row, index) => {
        value = roundValue(value * (1 + input.rates[index] / 100), currency.decimals);
  ...
  app.post("/api/budget-original/lines/:id/copy", (request: Request, response: Response) => {
    ...
    if (Number(source.company_id) !== Number(target.company_id) || Number(source.exercise_id) !== Number(target.exercise_id) || Number(source.version_id) !== Number(target.version_id)) {
      httpError("La línea de origen debe pertenecer a la misma empresa, ejercicio y versión.");
    }
    const stamp = new Date().toISOString();
    database.connection.transaction(() => {
      const months = database.connection.prepare(`SELECT source.budgeted_value, source.real_value, target.id AS target_id, p.status
        FROM budget_original_monthly_values source
        JOIN budget_original_monthly_values target ON target.period_id=source.period_id AND target.line_id=?
      ...
      for (const month of months) {
        ...
        database.connection.prepare("UPDATE budget_original_monthly_values SET budgeted_value=?,real_value=?,updated_at=? WHERE id=?")
          .run(month.budgeted_value, input.include_real ? month.real_value : null, stamp, month.target_id);
      }
      const projections = database.connection.prepare(`SELECT source.budgeted_value, source.real_value, target.id AS target_id
        FROM budget_original_projections source
        JOIN budget_original_projections target ON target.projection_year_id=source.projection_year_id AND target.line_id=?
        WHERE source.line_id=?`).all(targetId, input.source_line_id) as Array<Record<string, unknown>>;
      for (const projection of projections) {
        database.connection.prepare("UPDATE budget_original_projections SET budgeted_value=?,real_value=?,updated_at=? WHERE id=?")
          .run(projection.budgeted_value, input.include_real ? projection.real_value : null, stamp, projection.target_id);
      }
    })();
  ...
  app.post("/api/budget-original/approve", (request: Request, response: Response) => {
    ...
    if (Number(totals.complete_lines) !== totals.lines) httpError("Todas las líneas deben contener doce meses y tres años de proyección.", 409);
    const stamp = new Date().toISOString();
    database.connection.transaction(() => {
      database.connection.prepare("UPDATE budget_versions SET status='APROBADO',responsible_id=?,approved_at=?,updated_at=? WHERE id=?")
        .run(input.responsible_id, stamp, stamp, input.version_id);
      database.connection.prepare(`INSERT INTO version_status_history
        (company_id,version_id,from_status,to_status,responsible_id,notes,created_at) VALUES (?,?,'BORRADOR','APROBADO',?,?,?)`)
        .run(input.company_id, input.version_id, input.responsible_id, input.notes, stamp);
    })();
    audit(database, "APROBAR", "budget_versions", input.version_id, input.company_id, `Presupuesto original ${version.code} aprobado.`, version, { ...version, status: "APROBADO" });
