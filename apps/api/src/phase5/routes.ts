...
const monthSchema = z.object({
  ...
  budgeted_value: numberField,
  real_value: nullableNumberField,
});
...
function getDetailedLine(database: DatabaseManager, id: number) {
  const line = database.connection.prepare(`SELECT l.*,c.code center_code,c.name center_name,
    ...
    WHERE l.id=?`).get(id) as Record<string, unknown> | undefined;

  if (!line) httpError("La línea presupuestal no existe.", 404);

  const months = database.connection.prepare(`SELECT mv.*,p.period_number,p.name period_name,
    p.status period_status,p.start_date,p.end_date
  ...
  const annualBudget = months.reduce((sum, row) => sum + Number(row.budgeted_value ?? 0), 0);
  const comparableMonths = months.filter((row) => nullableNumber(row.real_value) !== null);
  const annualReal = comparableMonths.length
    ? comparableMonths.reduce((sum, row) => sum + Number(row.real_value), 0)
    : null;
  const comparableBudget = comparableMonths.length
    ? comparableMonths.reduce((sum, row) => sum + Number(row.budgeted_value ?? 0), 0)
    : null;
  ...
  return {
    ...
    projections,
    annual_budgeted: roundValue(annualBudget, decimals),
    annual_real: annualReal === null ? null : roundValue(annualReal, decimals),
    annual_comparable_budget: comparableBudget === null ? null : roundValue(comparableBudget, decimals),
    annual_variance: annualReal === null || comparableBudget === null
      ? null
      : roundValue(annualReal - comparableBudget, decimals),
    complete: months.length === 12 && projections.length === 3,
  };
}

function listLines(database: DatabaseManager, request: Request) {
  ...
  const rows = database.connection.prepare(`SELECT id FROM budget_original_lines
    WHERE ${where.join(" AND ")}
    ORDER BY center_id,group_id,element_id,account_id`).all(...params) as Array<{ id: number }>;
  return rows.map((row) => getDetailedLine(database, row.id));
}

export function registerOriginalBudgetRoutes(app: Express, database: DatabaseManager) {
  app.get("/api/budget-original/lines", (request, response) => {
    response.json(listLines(database, request));
  });

  app.get("/api/budget-original/lines/:id", (request, response) => {
    response.json(getDetailedLine(database, Number(request.params.id)));
  });

  app.get("/api/budget-original/summary", (request, response) => {
    const lines = listLines(database, request) as Array<Record<string, unknown>>;
    const totalBudget = lines.reduce((sum, row) => sum + Number(row.annual_budgeted ?? 0), 0);
    const linesWithReal = lines.filter((row) => row.annual_real !== null);
    const totalReal = linesWithReal.length
      ? linesWithReal.reduce((sum, row) => sum + Number(row.annual_real ?? 0), 0)
      : null;
    const comparableBudget = linesWithReal.length
      ? linesWithReal.reduce((sum, row) => sum + Number(row.annual_comparable_budget ?? 0), 0)
      : null;
    ...
    response.json({
      ...
      comparable_budget: comparableBudget,
      variance: totalReal === null || comparableBudget === null ? null : totalReal - comparableBudget,
  ...
  app.post("/api/budget-original/lines", (request: Request, response: Response) => {
    ...
    const id = database.connection.transaction(() => {
      const result = database.connection.prepare(`INSERT INTO budget_original_lines
        (company_id,exercise_id,version_id,center_id,group_id,element_id,account_id,currency_id,
         unit_id,responsible_id,comment,support,source_text,created_at,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
        input.company_id,
        input.exercise_id,
        input.version_id,
      ...
      const insertMonth = database.connection.prepare(`INSERT INTO budget_original_monthly_values
        (line_id,period_id,budgeted_value,real_value,created_at,updated_at)
        VALUES (?,?,0,NULL,?,?)`);
      ...
      const insertProjection = database.connection.prepare(`INSERT INTO budget_original_projections
        (line_id,projection_year_id,budgeted_value,real_value,created_at,updated_at)
        VALUES (?,?,0,NULL,?,?)`);
      years.forEach((year) => insertProjection.run(lineId, year.id, stamp, stamp));
      return lineId;
    })();
  ...
  app.patch("/api/budget-original/lines/:id", (request: Request, response: Response) => {
    ...
    const periods = new Map(getPeriods(database, companyId, exerciseId).map((row) => [Number(row.id), row]));
    const years = new Set(getProjectionYears(database, companyId, exerciseId).map((row) => Number(row.id)));
    const stamp = new Date().toISOString();
    ...
    database.connection.transaction(() => {
      for (const value of input.monthly_values ?? []) {
        ...
        if (changed && period.status === "CERRADO") {
      ...
      for (const value of input.projections ?? []) {
        if (!years.has(value.projection_year_id)) {
          httpError("Uno de los años proyectados no pertenece al ejercicio activo.");
        }
        const current = database.connection.prepare(`SELECT id,real_value FROM budget_original_projections
  ...
  app.post("/api/budget-original/lines/:id/distribute", (request: Request, response: Response) => {
    ...
    const rows = database.connection.prepare(`SELECT mv.*,p.status,p.period_number
      FROM budget_original_monthly_values mv
      JOIN budget_periods p ON p.id=mv.period_id
      WHERE mv.line_id=? ORDER BY p.period_number`).all(id) as Array<Record<string, unknown>>;
    const closedTotal = rows.filter((row) => row.status === "CERRADO")
      .reduce((sum, row) => sum + Number(row.budgeted_value), 0);
    const openRows = rows.filter((row) => row.status === "ABIERTO");
    if (!openRows.length) httpError("No existen periodos abiertos para distribuir el total anual.", 409);

    const remaining = input.annual_total - closedTotal;
    const share = roundValue(remaining / openRows.length, decimals);
    const stamp = new Date().toISOString();
    let assigned = 0;
    database.connection.transaction(() => {
      openRows.forEach((row, index) => {
        const amount = index === openRows.length - 1
          ? roundValue(remaining - assigned, decimals)
          : share;
        assigned += amount;
        database.connection.prepare(`UPDATE budget_original_monthly_values
          SET budgeted_value=?,updated_at=? WHERE id=?`).run(amount, stamp, row.id);
      });
    })();
  ...
  app.post("/api/budget-original/lines/:id/project", (request: Request, response: Response) => {
    const id = Number(request.params.id);
    const input = ratesSchema.parse(request.body);
    const line = ensureEditableLine(database, id);
    const decimals = (database.connection.prepare("SELECT decimals FROM currencies WHERE id=?").get(line.currency_id) as { decimals: number }).decimals;
    let value = Number((database.connection.prepare(`SELECT COALESCE(SUM(budgeted_value),0) total
      FROM budget_original_monthly_values WHERE line_id=?`).get(id) as { total: number }).total);
    const rows = database.connection.prepare(`SELECT bp.id,py.sequence
      FROM budget_original_projections bp
      JOIN projection_years py ON py.id=bp.projection_year_id
    ...
    database.connection.transaction(() => {
      rows.forEach((row, index) => {
        ...
        database.connection.prepare(`UPDATE budget_original_projections
          SET budgeted_value=?,updated_at=? WHERE id=?`).run(value, stamp, row.id);
      });
    })();
  ...
  app.post("/api/budget-original/lines/:id/copy", (request: Request, response: Response) => {
    ...
    if (source.company_id !== target.company_id
      || source.exercise_id !== target.exercise_id
    ...
    const stamp = new Date().toISOString();
    database.connection.transaction(() => {
      const months = database.connection.prepare(`SELECT s.budgeted_value,s.real_value,t.id target_id,p.status
        FROM budget_original_monthly_values s
        JOIN budget_original_monthly_values t ON t.period_id=s.period_id AND t.line_id=?
        JOIN budget_periods p ON p.id=s.period_id
        WHERE s.line_id=?`).all(targetId, input.source_line_id) as Array<Record<string, unknown>>;
      months.filter((row) => row.status !== "CERRADO").forEach((row) => {
        database.connection.prepare(`UPDATE budget_original_monthly_values
          SET budgeted_value=?,real_value=?,updated_at=? WHERE id=?`).run(
          row.budgeted_value,
          input.include_real ? row.real_value : null,
          stamp,
          row.target_id,
        );
      });

      const projections = database.connection.prepare(`SELECT s.budgeted_value,s.real_value,t.id target_id
        FROM budget_original_projections s
        JOIN budget_original_projections t ON t.projection_year_id=s.projection_year_id AND t.line_id=?
        WHERE s.line_id=?`).all(targetId, input.source_line_id) as Array<Record<string, unknown>>;
  ...
  app.post("/api/budget-original/approve", (request: Request, response: Response) => {
    ...
    const totals = database.connection.prepare(`SELECT COUNT(*) lines,
      SUM(CASE WHEN months=12 AND years=3 THEN 1 ELSE 0 END) complete
      FROM (SELECT l.id,
        (SELECT COUNT(*) FROM budget_original_monthly_values m WHERE m.line_id=l.id) months,
        (SELECT COUNT(*) FROM budget_original_projections p WHERE p.line_id=l.id) years
        FROM budget_original_lines l WHERE l.version_id=?)`).get(input.version_id) as { lines: number; complete: number | null };
    if (!totals.lines) httpError("Registre al menos una línea presupuestal antes de aprobar.", 409);
    if (Number(totals.complete) !== totals.lines) {
