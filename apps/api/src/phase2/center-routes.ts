...
export function registerCenterRoutes(app: Express, database: DatabaseManager) {
  app.get("/api/catalog/centros", (request: Request, response: Response) => {
  ...
  app.post("/api/catalog/centros", (request: Request, response: Response) => {
    const input = schema.parse(request.body); ensureReference(database, "sites", input.site_id, input.company_id, "La sede"); ensureReference(database, "responsibles", input.responsible_id, input.company_id, "El responsable");
    ...
    audit(database, "CREAR", "activity_centers", id, input.company_id, `Centro ${input.name} creado.`, undefined, input); response.status(201).json({ id, message: "Centro creado correctamente." });
  });
  app.patch("/api/catalog/centros/:id", (request: Request, response: Response) => {
    ...
    const companyId = Number(input.company_id ?? before.company_id); const values = { ...before, ...input, active: normalizeActive(input.active ?? before.active) } as Record<string, unknown>;
    ...
    audit(database, "EDITAR", "activity_centers", id, companyId, "Centro actualizado.", before, values); response.json({ message: "Centro actualizado correctamente." });
  });
  app.delete("/api/catalog/centros/:id", (request: Request, response: Response) => {
    ...
    audit(database, "ELIMINAR", "activity_centers", id, Number(before.company_id), "Centro desactivado.", before, { ...before, active: 0 }); response.json({ message: "Centro desactivado correctamente." });
  });
}
