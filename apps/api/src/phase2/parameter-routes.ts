...
import { activeField, audit, codeField, normalizeActive, nullableText, positiveId, requirePermission, text, type AuthenticatedRequest } from "./common";
...
export function registerParameterRoutes(app: Express, database: DatabaseManager) {
  ...
  app.post("/api/catalog/monedas", requirePermission("PARAMETROS:CREAR"), (request: AuthenticatedRequest, response: Response) => {
    ...
    const id = Number(result.lastInsertRowid); audit(database, request.identity!, "CREAR", "currencies", id, null, `Moneda ${input.code} creada.`, undefined, input); response.status(201).json({ id, message: "Moneda creada correctamente." });
  });
  app.patch("/api/catalog/monedas/:id", requirePermission("PARAMETROS:EDITAR"), (request: AuthenticatedRequest, response: Response) => {
    ...
    audit(database, request.identity!, "EDITAR", "currencies", id, null, "Moneda actualizada.", before, values); response.json({ message: "Moneda actualizada correctamente." });
  });
  ...
  app.post("/api/catalog/tipos-cambio", requirePermission("PARAMETROS:CREAR"), (request: AuthenticatedRequest, response: Response) => {
    ...
    const id = Number(result.lastInsertRowid); audit(database, request.identity!, "CREAR", "exchange_rates", id, null, `Tipo de cambio ${input.rate_date} creado.`, undefined, input); response.status(201).json({ id, message: "Tipo de cambio creado correctamente." });
  });
  app.patch("/api/catalog/tipos-cambio/:id", requirePermission("PARAMETROS:EDITAR"), (request: AuthenticatedRequest, response: Response) => {
    ...
    audit(database, request.identity!, "EDITAR", "exchange_rates", id, null, "Tipo de cambio actualizado.", before, values); response.json({ message: "Tipo de cambio actualizado correctamente." });
  });
  ...
  app.post("/api/catalog/unidades", requirePermission("PARAMETROS:CREAR"), (request: AuthenticatedRequest, response: Response) => {
    ...
    const id = Number(result.lastInsertRowid); audit(database, request.identity!, "CREAR", "units_of_measure", id, null, `Unidad ${input.code} creada.`, undefined, input); response.status(201).json({ id, message: "Unidad creada correctamente." });
  });
  app.patch("/api/catalog/unidades/:id", requirePermission("PARAMETROS:EDITAR"), (request: AuthenticatedRequest, response: Response) => {
    ...
    audit(database, request.identity!, "EDITAR", "units_of_measure", id, null, "Unidad actualizada.", before, values); response.json({ message: "Unidad actualizada correctamente." });
  });
...
function deactivateCurrency(database: DatabaseManager, request: AuthenticatedRequest, response: Response) {
  ...
  database.connection.prepare("UPDATE currencies SET active=0, updated_at=? WHERE id=?").run(new Date().toISOString(), id); audit(database, request.identity!, "ELIMINAR", "currencies", id, null, "Moneda desactivada.", before, { ...before, active: 0 }); response.json({ message: "Moneda desactivada correctamente." });
}
function deactivateRate(database: DatabaseManager, request: AuthenticatedRequest, response: Response) {
  ...
  database.connection.prepare("UPDATE exchange_rates SET active=0, updated_at=? WHERE id=?").run(new Date().toISOString(), id); audit(database, request.identity!, "ELIMINAR", "exchange_rates", id, null, "Tipo de cambio desactivado.", before, { ...before, active: 0 }); response.json({ message: "Tipo de cambio desactivado correctamente." });
}
function deactivateUnit(database: DatabaseManager, request: AuthenticatedRequest, response: Response) {
  ...
  database.connection.prepare("UPDATE units_of_measure SET active=0, updated_at=? WHERE id=?").run(new Date().toISOString(), id); audit(database, request.identity!, "ELIMINAR", "units_of_measure", id, null, "Unidad desactivada.", before, { ...before, active: 0 }); response.json({ message: "Unidad desactivada correctamente." });
}
