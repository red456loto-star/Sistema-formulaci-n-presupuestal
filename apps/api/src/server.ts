...
export interface StartedServer { port: number; host: string; url: string; server: http.Server; database: DatabaseManager; close: () => Promise<void>; }
...
function releaseMetadata() {
  const candidate = Number(process.env.PRESUCONTROL_COMPAT_PHASE ?? 12);
  return { version: process.env.PRESUCONTROL_COMPAT_VERSION || "0.12.0", phase: Number.isInteger(candidate) && candidate > 0 ? candidate : 12 };
}
...
export function createApp(options: StartServerOptions = {}) {
  ...
  ensurePhase4Schema(database); ensurePhase5Schema(database); ensurePhase6Schema(database); ensurePhase7Schema(database); ensurePhase8Schema(database); ensurePhase9Schema(database); ensurePhase10Schema(database); ensurePhase11Schema(database);
  const app = express();
  ...
  app.get("/api/health", (_request, response) => {
    ...
    response.json({ status: "ok", service: "presucontrol-api", version: release.version, phase: release.phase, accessMode: "directo", timestamp: new Date().toISOString(), database: database.getStatus().connected ? "conectada" : "no disponible" });
  });
  ...
  app.get("/api/demo/summary", (_request, response) => {
    ...
    response.json({
      ...
      propuestas_corr: count("phase11_improvement_proposals"),
      mensaje: "Corrección activa: tipos de presupuesto separados del presupuesto maestro y datos maestros por contexto.",
    });
  });

  registerCompanyRoutes(app, database); registerCenterRoutes(app, database); registerGroupElementRoutes(app, database); registerAccountRoutes(app, database); registerParameterRoutes(app, database); registerOrganizationRoutes(app, database);
  registerExerciseRoutes(app, database); registerVersionRoutes(app, database); registerImportRoutes(app, database); registerOriginalBudgetRoutes(app, database); registerMasterBudgetRoutes(app, database);
  registerPhase7Routes(app, database); registerFinancialAnalysisContextGuard(app, database); registerFinancialAnalysisCorrectionRoutes(app, database); registerFinancialAnalysisRoutes(app, database);
  registerPhase9Routes(app, database); registerPhase10Routes(app, database); registerPhase11ManualAppendRoute(app, database); registerPhase11Routes(app, database);

  app.get("/api/system/database-status", (_request, response) => response.json(database.getStatus()));
  app.post("/api/system/backup", async (_request, response, next) => { try { const backupPath = await database.createBackup(); response.status(201).json({ message: `Respaldo creado correctamente: ${path.basename(backupPath)}`, path: backupPath }); } catch (error) { next(error); } });
  app.post("/api/system/restore-latest", async (_request, response, next) => { try { const restoredPath = database.restoreLatestBackup(); response.json({ message: `Base restaurada desde ${path.basename(restoredPath)}.`, path: restoredPath }); } catch (error) { next(error); } });
  ...
  const errorHandler: ErrorRequestHandler = (error, _request: Request, response: Response, _next) => {
    ...
    if (error instanceof z.ZodError) { response.status(400).json({ code: "VALIDATION_ERROR", message: "La información enviada no es válida.", details: error.flatten() } satisfies ApiErrorResponse); return; }
    const message = error instanceof Error ? error.message : "Se produjo un error interno.";
    const statusCode = Number((error as { statusCode?: number })?.statusCode || 500);
    if (message.includes("UNIQUE constraint failed")) { response.status(409).json({ code: "DUPLICATE", message: "Ya existe un registro con la misma combinación dentro del contexto activo." } satisfies ApiErrorResponse); return; }
    if (message.includes("CHECK constraint failed")) { response.status(400).json({ code: "INVALID_VALUE", message: "Uno de los valores no cumple las reglas permitidas." } satisfies ApiErrorResponse); return; }
    if (message.includes("FOREIGN KEY constraint failed")) { response.status(409).json({ code: "RELATED_DATA", message: "La operación no puede completarse porque existen datos relacionados." } satisfies ApiErrorResponse); return; }
...
export async function startServer(options: StartServerOptions = {}): Promise<StartedServer> {
  ...
  const server = await new Promise<http.Server>((resolve, reject) => { const instance = app.listen(requestedPort, host, () => resolve(instance)); instance.once("error", reject); });
  ...
  return { port, host, url, server, database, close: async () => { await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve())); database.close(); logger.flush(); } };
}
