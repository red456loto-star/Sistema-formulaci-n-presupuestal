...
const templateColumns = [
  ["center_code", "Código del centro"], ["center_name", "Nombre del centro"],
  ["element_code", "Código del elemento"], ["element_name", "Nombre del elemento"],
  ["account_code", "Código de cuenta"], ["account_name", "Nombre de cuenta"],
  ["account_nature", "Naturaleza: INGRESO/COSTO/GASTO/ACTIVO/PASIVO/PATRIMONIO"],
  ["line_code", "Código de línea"], ["line_name", "Nombre de línea"],
  ["statement_section", "PRESUPUESTO/ESTADO_RESULTADOS/ESTADO_SITUACION/FLUJO_EFECTIVO"],
  ["financial_item", "Partida financiera normalizada"], ["cost_behavior", "FIJO/VARIABLE/NO_APLICA"],
  ["cost_traceability", "DIRECTO/INDIRECTO/NO_APLICA"], ["quantity", "Cantidad"],
  ["unit_price", "Precio unitario"], ["amount", "Importe"],
  ["source_reference", "Fuente o referencia"], ["notes", "Observaciones"],
] as const;
...
const aliases: Record<string, string[]> = {
  ...
  source_reference: ["source_reference", "fuente", "referencia"], notes: ["notes", "observaciones", "nota"],
};
...
function parseSheet(sheet: ExcelJS.Worksheet) {
  ...
  for (let rowNumber = headerRow + 1; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    rows.push({
      ...
      account_code: optionalText(get("account_code")), account_name: optionalText(get("account_name")), account_nature: accountNature,
      line_code: optionalText(get("line_code")), line_name: lineName ?? "", statement_section: statementSection,
      financial_item: optionalText(get("financial_item"))?.toUpperCase().replace(/\s+/g, "_") ?? null,
...
export async function inspectMasterWorkbook(fileName: string, contentBase64: string, sheetName?: string | null) {
  ...
  return {
    file_name: fileName, sheet_name: sheet.name,
...
export async function masterTemplate() {
  ...
  sheet.addRow({
    ...
    account_code: "7011", account_name: "Ventas", account_nature: "INGRESO", line_code: "VENTAS", line_name: "Ventas netas",
    statement_section: "ESTADO_RESULTADOS", financial_item: "SALES", cost_behavior: "NO_APLICA", cost_traceability: "NO_APLICA",
    amount: 100000, source_reference: "Fuente documentada",
  });
...
function resolveDimension(database: DatabaseManager, table: string, companyId: number, code?: string | null) {
  if (!code) return null;
  return database.connection.prepare(`SELECT id,name FROM ${table} WHERE company_id=? AND UPPER(code)=UPPER(?) AND active=1`)
    .get(companyId, code) as { id: number; name: string } | undefined ?? null;
}
...
function ensureDataset(database: DatabaseManager, input: MasterDataImportInput) {
  ...
  if (existing && !input.replace_existing) httpError(`Ya existe información ${input.data_kind.toLowerCase()} para esta combinación. Elimine el registro o active la sustitución.`, 409);
  if (existing) {
...
export function importMasterData(database: DatabaseManager, input: MasterDataImportInput) {
  ...
  if (input.rows.some((row) => !row.line_name.trim() || !Number.isFinite(Number(row.amount)))) httpError("Todas las filas deben tener nombre de línea e importe numérico.", 400);
  return database.connection.transaction(() => {
    ...
    return { dataset_id: datasetId, created: input.rows.length, message: `Se registraron ${input.rows.length} filas ${input.data_kind.toLowerCase()} para el contexto activo.` };
  })();
}

export function listMasterData(database: DatabaseManager, input: Phase11ContextInput) {
  ensurePhase11Context(database, input);
  const params = [input.company_id, input.exercise_id, input.period_id, input.version_id, input.budget_type_id];
  const datasets = database.connection.prepare(`SELECT d.*,bt.code budget_type_code,bt.name budget_type_name,p.period_number,p.name period_name,v.code version_code,v.name version_name,
    ...
    GROUP BY d.id ORDER BY CASE d.data_kind WHEN 'PRESUPUESTADO' THEN 1 ELSE 2 END`).all(...params);
  const rows = database.connection.prepare(`SELECT r.*,d.source_file,d.source_label,d.wacc_rate,bt.name budget_type_name,
    ...
    ORDER BY CASE r.data_kind WHEN 'PRESUPUESTADO' THEN 1 ELSE 2 END,r.row_order,r.id`).all(...params);
...
export function updateMasterRow(database: DatabaseManager, id: number, patch: Partial<MasterDataRowInput>) {
  ...
  const center = resolveDimension(database, "activity_centers", companyId, optionalText(merged.center_code));
  const element = resolveDimension(database, "budget_elements", companyId, optionalText(merged.element_code));
  const account = resolveDimension(database, "budget_accounts", companyId, optionalText(merged.account_code));
