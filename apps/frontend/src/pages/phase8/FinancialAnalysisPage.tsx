...
export function FinancialAnalysisPage() {
  ...
  const saveMappings = async () => {
    ...
    setBusy(true); setError(""); setSuccess("");
    try {
      const selected = mappings.map((row) => {
        const draft = mappingDrafts[row.account_id] ?? EMPTY_MAPPING;
        return {
          account_id: row.account_id,
          statement_section: draft.statement_section || null,
          ratio_role: draft.statement_section === "CURRENT_ASSET" ? (draft.ratio_role || null) : null,
  ...
  const makeSideDescriptor = (side: CompareSide): AnalysisDescriptor => {
    if (!companyId || !side.exercise_id || !side.version_id) throw new Error("Complete los escenarios inicial y final.");
    return {
      company_id: companyId,
      exercise_id: Number(side.exercise_id),
      version_id: Number(side.version_id),
      source_type: side.source_type,
      period_number: side.period_number ? Number(side.period_number) : null,
    };
  };
  ...
  const compare = async () => {
    ...
    try {
      setHorizontal(await apiRequest<HorizontalResult>("/api/financial-analysis/horizontal", {
        method: "POST",
        body: JSON.stringify({ initial: makeSideDescriptor(initialSide), final: makeSideDescriptor(finalSide) }),
      }));
  ...
  const exportHorizontal = async () => {
    ...
    setBusy(true); setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/financial-analysis/horizontal/export`, {
        method: "POST",
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => null) as { message?: string } | null;
        throw new Error(body?.message ?? "No fue posible exportar la comparación horizontal.");
      }
      const url = URL.createObjectURL(await response.blob());
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = `analisis-horizontal-${horizontal.initial.version_code}-${horizontal.final.version_code}.xlsx`;
      document.body.appendChild(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(url);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "No fue posible exportar la comparación horizontal."); }
    finally { setBusy(false); }
  };

  const currency = snapshot?.context.currency_code ?? exercise?.currency_code ?? "PEN";
  return <div className="page-stack">
    ...
    {!companyId || !exerciseId || !versionId ? <Message>Seleccione empresa, ejercicio y versión en la barra superior.</Message> : <>
      ...
      {snapshot && <>
        <div className="analysis-status-line"><strong>{snapshot.context.source_type} · {snapshot.context.version_code}</strong><span>{snapshot.context.period_label} · {snapshot.context.currency_code}</span><span className={`status-pill ${snapshot.complete ? "status-pill--success" : "status-pill--warning"}`}>{snapshot.complete ? "Análisis completo" : "Requiere revisión"}</span></div>
        ...
        {tab === "Análisis horizontal" && <>
          <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Comparación</span><h2>Escenario inicial y escenario final</h2><p>Puede comparar periodos, versiones y ejercicios de la misma empresa siempre que utilicen la misma moneda.</p></div></div><div className="horizontal-selectors"><CompareSelector title="Inicial" side={initialSide} setSide={setInitialSide} exercises={exercises} versions={initialVersions} onExercise={(value) => void loadSideVersions("initial", value)} onChange={() => setHorizontal(null)} /><CompareSelector title="Final" side={finalSide} setSide={setFinalSide} exercises={exercises} versions={finalVersions} onExercise={(value) => void loadSideVersions("final", value)} onChange={() => setHorizontal(null)} /></div><div className="button-row"><button className="button button--primary" disabled={busy} onClick={() => void compare()}>Comparar escenarios</button><button className="button button--ghost" disabled={busy || !horizontal} onClick={() => void exportHorizontal()}><Download size={16} />Exportar comparación</button></div></section>
          <HorizontalView result={horizontal} currency={horizontal?.initial.currency_code ?? currency} />
        </>}
...
function ConfigurationPanel({ assumptions, form, setForm, mappings, drafts, setDrafts, busy, onSaveAssumptions, onSaveMappings }: {
  ...
  return <>
    <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Supuestos visibles</span><h2>Variables para impuesto y EVA</h2><p>Las variables faltantes permanecen vacías; el sistema no las inventa. El análisis solo se marca completo cuando el EVA puede calcularse.</p></div></div><FormGrid>
    ...
    <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Clasificación contable</span><h2>Mapeo de cuentas para forecast y real</h2><p>Seleccione “Sin clasificar” y guarde para eliminar una clasificación anterior.</p></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Cuenta</th><th>Naturaleza</th><th>Partida financiera</th><th>Rol para ratios</th><th>Notas</th></tr></thead><tbody>{mappings.map((row) => {
...
function CompareSelector({ title, side, setSide, exercises, versions, onExercise, onChange }: {
  title: string;
  side: CompareSide;
  ...
  return <div className="compare-box"><h3>{title}</h3><Field label="Ejercicio"><select value={side.exercise_id} onChange={(event) => { onChange(); onExercise(event.target.value); }}><option value="">Seleccione</option>{exercises.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.budget_year} · {item.currency_code}</option>)}</select></Field><Field label="Versión"><select value={side.version_id} onChange={(event) => { onChange(); const next = versions.find((item) => item.id === Number(event.target.value)); setSide((current) => ({ ...current, version_id: event.target.value, source_type: next?.version_type === "FORECAST" ? "FORECAST" : "ORIGINAL" })); }}><option value="">Seleccione</option>{versions.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.version_type}</option>)}</select></Field><Field label="Fuente"><select value={side.source_type} onChange={(event) => { onChange(); setSide((current) => ({ ...current, source_type: event.target.value as AnalysisSourceType })); }}>{sourceOptions(selectedVersion).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field><Field label="Periodo"><select value={side.period_number} onChange={(event) => { onChange(); setSide((current) => ({ ...current, period_number: event.target.value })); }}><option value="">Total anual / cierre</option>{months.map((name, index) => <option key={name} value={index + 1}>{index + 1} · {name}</option>)}</select></Field></div>;
}
