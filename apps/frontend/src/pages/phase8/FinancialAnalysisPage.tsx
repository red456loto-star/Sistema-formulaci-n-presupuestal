...
function descriptorQuery(descriptor: AnalysisDescriptor) {
...
function emptyAssumptions(): AnalysisAssumptions {
...
export function FinancialAnalysisPage() {
  const { companyId, exerciseId, exercise, versionId, version, versions, exercises } = useWorkspace();
  ...
  const [mappingDrafts, setMappingDrafts] = useState<Record<number, { statement_section: string; ratio_role: string; notes: string }>>({});
  const [assumptions, setAssumptions] = useState<AnalysisAssumptions>(emptyAssumptions());
  ...
  useEffect(() => {
    const next = version?.version_type === "FORECAST" ? "FORECAST" : "ORIGINAL";
    setSourceType(next);
    setSnapshot(null);
  ...
  const loadReport = async () => {
    ...
    try {
      const result = await apiRequest<FinancialSnapshot>(`/api/financial-analysis/report?${descriptorQuery(descriptor)}`);
      setSnapshot(result);
  ...
  const saveMappings = async () => {
    ...
    setBusy(true); setError(""); setSuccess("");
    try {
      const selected = mappings.filter((row) => mappingDrafts[row.account_id]?.statement_section).map((row) => ({
        account_id: row.account_id,
        statement_section: mappingDrafts[row.account_id].statement_section,
        ratio_role: mappingDrafts[row.account_id].ratio_role || null,
        notes: mappingDrafts[row.account_id].notes || null,
      }));
      const result = await apiRequest<{ message: string }>("/api/financial-analysis/mappings", { method: "PUT", body: JSON.stringify({ company_id: companyId, mappings: selected }) });
      setSuccess(result.message); await loadMappings(); await loadReport();
  ...
  const saveAssumptions = async () => {
    ...
    try {
      ...
      setSuccess(result.message); await loadReport();
  ...
  const exportExcel = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/financial-analysis/export?${descriptorQuery(descriptor)}`);
      if (!response.ok) {
  ...
  const loadVersionsForSide = async (side: "initial" | "final", exerciseValue: string) => {
    if (!companyId || !exerciseValue) return;
    const rows = await apiRequest<BudgetVersion[]>(`/api/catalog/versiones?company_id=${companyId}&exercise_id=${exerciseValue}`);
    const first = rows[0];
    if (side === "initial") {
      setInitialVersions(rows);
      setInitialSide((current) => ({ ...current, exercise_id: exerciseValue, version_id: String(first?.id ?? ""), source_type: first?.version_type === "FORECAST" ? "FORECAST" : "ORIGINAL" }));
    } else {
      setFinalVersions(rows);
      setFinalSide((current) => ({ ...current, exercise_id: exerciseValue, version_id: String(first?.id ?? ""), source_type: first?.version_type === "FORECAST" ? "FORECAST" : "ORIGINAL" }));
    }
  };
  ...
  useEffect(() => {
    ...
    void loadVersionsForSide("initial", value);
    void loadVersionsForSide("final", value);
  ...
  const compare = async () => {
    ...
    setBusy(true); setError(""); setSuccess("");
    try {
      const sideDescriptor = (side: CompareSide): AnalysisDescriptor => {
      ...
      const result = await apiRequest<HorizontalResult>("/api/financial-analysis/horizontal", { method: "POST", body: JSON.stringify({ initial: sideDescriptor(initialSide), final: sideDescriptor(finalSide) }) });
      setHorizontal(result);
  ...
  const availableSources = sourceOptions(version);
  ...
  return <div className="page-stack">
    ...
    {!companyId || !exerciseId || !versionId ? <Message>Seleccione empresa, ejercicio y versión en la barra superior.</Message> : <>
      <section className="panel analysis-context"><FormGrid>
        <Field label="Fuente de análisis"><select value={sourceType} onChange={(event) => setSourceType(event.target.value as AnalysisSourceType)}>{availableSources.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field>
        <Field label="Periodo"><select value={periodNumber} onChange={(event) => setPeriodNumber(event.target.value)}><option value="">Total anual / cierre</option>{monthOptions.map((name, index) => <option key={name} value={index + 1}>{index + 1} · {name}</option>)}</select></Field>
      ...
      {snapshot && <>
        ...
        {tab === "Análisis horizontal" && <>
          <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Comparación</span><h2>Escenario inicial y escenario final</h2><p>Puede comparar periodos, versiones y ejercicios de la misma empresa.</p></div></div><div className="horizontal-selectors"><CompareSelector title="Inicial" side={initialSide} setSide={setInitialSide} exercises={exercises} versions={initialVersions} onExercise={(value) => void loadVersionsForSide("initial", value)} /><CompareSelector title="Final" side={finalSide} setSide={setFinalSide} exercises={exercises} versions={finalVersions} onExercise={(value) => void loadVersionsForSide("final", value)} /></div><div className="button-row"><button className="button button--primary" disabled={busy} onClick={() => void compare()}>Comparar escenarios</button></div></section>
        ...
        {tab === "Configuración" && <>
          <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Supuestos visibles</span><h2>Variables para impuesto y EVA</h2><p>Las variables faltantes permanecen vacías; el sistema no las inventa.</p></div></div><FormGrid>
            <Field label="Tasa de impuesto (%)"><input type="number" step="0.01" value={assumptionForm.tax_rate} onChange={(event) => setAssumptionForm((current) => ({ ...current, tax_rate: event.target.value }))} /></Field>
            <Field label="Costo de capital (%)"><input type="number" step="0.01" value={assumptionForm.cost_of_capital_rate} onChange={(event) => setAssumptionForm((current) => ({ ...current, cost_of_capital_rate: event.target.value }))} /></Field>
            <Field label="Capital invertido manual"><input type="number" step="0.01" value={assumptionForm.invested_capital_override} placeholder="Vacío = activos - pasivos corrientes" onChange={(event) => setAssumptionForm((current) => ({ ...current, invested_capital_override: event.target.value }))} /></Field>
            <Field label="Fuente de los supuestos" span={2}><input value={assumptionForm.source_reference} onChange={(event) => setAssumptionForm((current) => ({ ...current, source_reference: event.target.value }))} /></Field>
            <Field label="Notas" span={2}><textarea value={assumptionForm.notes} onChange={(event) => setAssumptionForm((current) => ({ ...current, notes: event.target.value }))} /></Field>
          </FormGrid><div className="button-row"><button className="button button--primary" disabled={busy} onClick={() => void saveAssumptions()}><Save size={16} />Guardar supuestos</button></div><p className="muted">Estado: {assumptions.saved ? "supuestos guardados para este contexto" : "valores no guardados o derivados de una fase anterior"}.</p></section>
          <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Clasificación contable</span><h2>Mapeo de cuentas para forecast y real</h2><p>Es indispensable para distinguir partidas de resultados, activos, pasivos y patrimonio.</p></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Cuenta</th><th>Naturaleza</th><th>Partida financiera</th><th>Rol para ratios</th><th>Notas</th></tr></thead><tbody>{mappings.map((row) => { const draft = mappingDrafts[row.account_id] ?? { statement_section: "", ratio_role: "", notes: "" }; return <tr key={row.account_id}><td>{row.account_code} · {row.account_name}<br /><span className="muted">{row.group_code} / {row.element_code}</span></td><td>{row.account_nature}</td><td><select value={draft.statement_section} onChange={(event) => setMappingDrafts((current) => ({ ...current, [row.account_id]: { ...draft, statement_section: event.target.value, ratio_role: event.target.value === "CURRENT_ASSET" ? draft.ratio_role : "" } }))}>{sectionOptions.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></td><td><select disabled={draft.statement_section !== "CURRENT_ASSET"} value={draft.ratio_role} onChange={(event) => setMappingDrafts((current) => ({ ...current, [row.account_id]: { ...draft, ratio_role: event.target.value } }))}>{roleOptions.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></td><td><input value={draft.notes} onChange={(event) => setMappingDrafts((current) => ({ ...current, [row.account_id]: { ...draft, notes: event.target.value } }))} /></td></tr>; })}</tbody></table></div><div className="button-row"><button className="button button--primary" disabled={busy} onClick={() => void saveMappings()}><Save size={16} />Guardar clasificaciones</button></div></section>
        </>}
      </>}
    </>}
  </div>;
...
function CompareSelector({ title, side, setSide, exercises, versions, onExercise }: {
  title: string;
  side: CompareSide;
  setSide: React.Dispatch<React.SetStateAction<CompareSide>>;
  ...
  const sources = sourceOptions(selectedVersion);
  return <div className="compare-box"><h3>{title}</h3><Field label="Ejercicio"><select value={side.exercise_id} onChange={(event) => onExercise(event.target.value)}><option value="">Seleccione</option>{exercises.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.budget_year}</option>)}</select></Field><Field label="Versión"><select value={side.version_id} onChange={(event) => { const next = versions.find((item) => item.id === Number(event.target.value)); setSide((current) => ({ ...current, version_id: event.target.value, source_type: next?.version_type === "FORECAST" ? "FORECAST" : "ORIGINAL" })); }}><option value="">Seleccione</option>{versions.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.version_type}</option>)}</select></Field><Field label="Fuente"><select value={side.source_type} onChange={(event) => setSide((current) => ({ ...current, source_type: event.target.value as AnalysisSourceType }))}>{sources.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field><Field label="Periodo"><select value={side.period_number} onChange={(event) => setSide((current) => ({ ...current, period_number: event.target.value }))}><option value="">Total anual / cierre</option>{monthOptions.map((name, index) => <option key={name} value={index + 1}>{index + 1} · {name}</option>)}</select></Field></div>;
}
