...
import { Building2, CalendarDays, CheckCircle2, Database, FileClock, Server, WifiOff } from "lucide-react";
...
export function DashboardPage() {
  ...
  return <div className="page-stack">
    <section className="page-heading"><div><span className="eyebrow">Fase 3 · Multiperiodos y multiversiones</span><h1>Panel de inicio</h1><p>La aplicación organiza el contexto activo por empresa, ejercicio, periodo y versión, manteniendo acceso local directo y sin autenticación.</p></div><span className={`status-pill ${error ? "status-pill--danger" : "status-pill--success"}`}>{error ? <WifiOff size={16} /> : <CheckCircle2 size={16} />}{error ? "API no disponible" : "Servicios locales activos"}</span></section>
    {error && <div className="alert alert--danger"><strong>No se pudo consultar la API local.</strong><span>{error}</span></div>}
    <section className="metric-grid"><MetricCard label="Empresas" value={summary?.empresas ?? 0} icon={Building2} /><MetricCard label="Ejercicios" value={summary?.ejercicios ?? 0} icon={CalendarDays} /><MetricCard label="Periodos" value={summary?.periodos ?? 0} icon={CalendarDays} /><MetricCard label="Versiones" value={summary?.versiones ?? 0} icon={FileClock} /></section>
    <section className="grid-2"><article className="panel"><div className="panel__heading"><div><span className="eyebrow">Estado técnico</span><h2>Comunicación local</h2></div><Server size={22} /></div><dl className="detail-list"><div><dt>API local</dt><dd>{API_BASE_URL}</dd></div><div><dt>Servicio</dt><dd>{health?.service ?? "Consultando..."}</dd></div><div><dt>Versión</dt><dd>{health?.version ?? "—"}</dd></div><div><dt>Fase</dt><dd>{health?.phase ?? "—"}</dd></div><div><dt>SQLite</dt><dd>{health?.database ?? "Consultando..."}</dd></div><div><dt>Acceso</dt><dd>Directo, sin login</dd></div></dl></article><article className="panel panel--accent"><div className="panel__heading"><div><span className="eyebrow">Alcance actual</span><h2>Qué contiene la Fase 3</h2></div><Database size={22} /></div><ul className="check-list"><li>Ejercicios presupuestales por empresa.</li><li>Doce periodos mensuales generados automáticamente.</li><li>Tres años posteriores de proyección.</li><li>Versiones originales y forecast.</li><li>Aprobación, cierre, reemplazo e historial.</li><li>Selectores superiores persistentes y dependientes.</li></ul></article></section>
    <section className="panel"><div className="panel__heading"><div><span className="eyebrow">Contexto activo</span><h2>{company?.commercial_name ?? "Seleccione una empresa"}</h2></div><Database size={22} /></div><dl className="detail-list"><div><dt>Ejercicio</dt><dd>{exercise ? `${exercise.code} · ${exercise.budget_year}` : "Sin seleccionar"}</dd></div><div><dt>Periodo</dt><dd>{period ? `${period.period_number}. ${period.name} · ${period.status}` : "Sin seleccionar"}</dd></div><div><dt>Versión</dt><dd>{version ? `${version.code} · ${version.name} · ${version.status}` : "Sin seleccionar"}</dd></div></dl><p className="muted">Los importes presupuestales permanecen fuera del alcance de esta fase.</p></section>
  </div>;
}
